...
cell_type_t cell_type_from_string(std::string x)
{
    // TODO: Think about replacing with constexpr map lookup
    size_t sdff_pos = x.find("SDFF");
    if (sdff_pos != std::string::npos)
        x = x.replace(sdff_pos, 4, "DFF", 3);
    ...
    else if (x == "$_ANDNOT_") return cell_type_t::CELL_ANDNOT;
    else if (x == "$_ORNOT_") return cell_type_t::CELL_ORNOT;
